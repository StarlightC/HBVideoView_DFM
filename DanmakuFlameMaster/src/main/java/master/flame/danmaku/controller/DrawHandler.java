...
public class DrawHandler extends Handler {
    ...
    private static final int PAUSE_SEEK_POS = 16;
    ...
    public void refresh() {
        obtainMessage(REFRESH, quitFlag);
    }
    ...
    public void handleMessage(Message msg) {
        ...
        switch (what) {
            ...
            case PAUSE_SEEK_POS:
            case SEEK_POS:
                if (what == SEEK_POS || what == PAUSE_SEEK_POS) {
                    quitFlag = true;
                    quitUpdateThread();
            ...
            case UPDATE:
                ...
                break;
            case REFRESH:
                boolean originState = (boolean) msg.obj;
                quitFlag = false;
                if (mContext.updateMethod == 0) {
                    updateInChoreographer();
                } else if (mContext.updateMethod == 1) {
...
//            Log.e("DrawHandler", time+"|d:" + d  + "RemaingTime:" + mRemainingTime + ",gapTime:" + gapTime + ",rtim:" + mRenderingState.consumingTime + ",average:" + averageTime);
    ...
    private void initRenderingConfigs() {
        ...
        mFrameUpdateRate = getConfig().getRefreshTime();
...
//                + mCordonTime + ",mFrameRefreshingRate:" + mFrameUpdateRate);
    ...
    public void removeDanmaku(BaseDanmaku danmaku) {
        if (drawTask != null) {
            drawTask.removeDanmaku(danmaku);
        }
    }
    ...
    public IDanmakus getPreAndCurrentDanmakus() {
        if (drawTask != null) {
            return drawTask.getPreAndCurrentDanmakus(getCurrentTime());
        }
        return null;
    }
    ...
    public IDanmakus getAllDanmakus() {
        if (drawTask != null) {
            return drawTask.getAllDanmakus();
        }
        return null;
    }
