...
public abstract class BaseDanmaku {
     ...
     * 水平内边距(像素)
     */
    public int paddingHor = 0;

    /**
     * 纵向内边距(px)
     */
    public int paddingVer = 0;
     ...
     * 隐藏
     */
    public boolean hideThisTime = false;
    ...
    public boolean isTimeOut(long ctime) {
        Boolean realTimeOut = ctime - getActualTime() >= duration.value;
        if (hideThisTime) {
            if (realTimeOut) {
                hideThisTime = false;
            }
