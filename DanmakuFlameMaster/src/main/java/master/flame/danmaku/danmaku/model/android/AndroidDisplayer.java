...
public class AndroidDisplayer extends AbsDisplayer<Canvas, Typeface> {
    ...
    public static class DisplayerConfig {
        ...
        private void applyTextScaleConfig(BaseDanmaku danmaku, Paint paint) {
            ...
            Float size;
            if (isFullscreen){
                size = sCachedScaleSize.get(danmaku.textSize + danmaku.sizeGap);
            } else {
                size = sCachedScaleSize.get(danmaku.textSize);
            }
            if (size == null || sLastScaleTextSize != scaleTextSize) {
                sLastScaleTextSize = scaleTextSize;
                if (isFullscreen) {
                    size = (danmaku.textSize + danmaku.sizeGap) * scaleTextSize;
                    sCachedScaleSize.put(danmaku.textSize + danmaku.sizeGap, size);
                } else {
        ...
        public TextPaint getPaint(BaseDanmaku danmaku, boolean fromWorkerThread) {
            ...
            if (isFullscreen) {
                paint.setTextSize(danmaku.textSize + danmaku.sizeGap);
            } else {
    ...
    private void setDanmakuPaintWidthAndHeight(BaseDanmaku danmaku, float w, float h) {
        float pw = w + 2 * danmaku.paddingHor;
        float ph = h + 2 * danmaku.paddingVer;
    ...
    @Override
    public DisplayerConfig getDisplayerConfig() {
